...
<script>
import { freeOrderStrategy } from '@/api/http'
export default {
  data() {
    return {
      ruleForm: {
      ...
      result: '',
      desc: '<span>随机免单<br>czb_activity库<br>free_order_strategy   随机免单策略<br>free_gas_rel_group   随机免单油站组油站编码关系表<br>free_gas_group 随机免单油站组<br><br>业务创建步骤：<br>赫拉=&gt;智能营销=&gt;油站分组_随机免单 下面创建一个油站组，指定油站id<br>然后在 随机免单tab下 创建策略  必填项：基础命中率 为100  查看 数据库 czb_activity.free_order_strategy表里 base_rate 字段必须 1.000  此时下单必免单。<br></span>',
    }
  },
  methods: {
    submitForm(formName) {
      this.$refs[formName].validate(async (valid) => {
        if (!valid) return
        let result = await freeOrderStrategy(this.ruleForm)
